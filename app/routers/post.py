...
async def get_posts(db: Session = Depends(get_db), current_user: int = Depends(ouath2.get_current_user), limit: int = 100, skip: int = 0, search: Optional[str] = ""):
...
def create_posts(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(ouath2.get_current_user)):
    ...
    new_post = models.Post(owner_id=current_user.id, **post.dict()) # It replaces the next line
...
def update_post(id: int, updated_post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(ouath2.get_current_user)):
